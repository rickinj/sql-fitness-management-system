...
CREATE PROCEDURE `sp_total_membership_revenue` ()
BEGIN
    ...
    BEGIN
        ...
        SELECT -1 AS Status , @p2 AS Message , 'sp_total_membership_revenue' , 'DB', NOW() FROM DUAL;
        SET v_returncode = -1;
        SELECT v_returncode AS ret_status;
    END;

    SELECT SUM(ms.price) AS total_revenue
    FROM tbl_member_membership mm
    INNER JOIN tbl_memberships ms
